	...
	tests := []struct {
	...
	}{
		{
			...
			sqlExpected: []string{
				...
				"CREATE TABLE IF NOT EXISTS Orders \\( order_id int PRIMARY KEY, login text NOT NULL, status text NOT NULL, score int NOT NULL, last_changed timestamp NOT NULL \\)",
				"CREATE TABLE IF NOT EXISTS Withdrawals \\( order_id int PRIMARY KEY, login text NOT NULL, wd int NOT NULL, time timestamp NOT NULL \\)",
			},
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			want: []string{
				...
				`INSERT INTO Orders \(order_id, login, status, score, last_changed\) VALUES \(\$1, \$2, \$3, \$4, \$5\)`,
				`UPDATE Orders SET status = \$3, score = \$4, last_changed = \$5 WHERE login = \$2 AND order_id = \$1`,
				`SELECT order_id, login, status, score, last_changed FROM Orders WHERE login = \$1`,
